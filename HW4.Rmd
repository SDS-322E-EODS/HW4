...
First load the `tidyverse` package so that we can have access to the `tidyverse` data transformation functions. We also want to look at the `penguins` dataset.

```{r message=FALSE}

## There is no need to edit this code block.
# Load tidyverse
library(tidyverse)

# Save the data as a dataframe
penguins <- tibble(datasets::penguins)

# Take a look at the top of the data frame 
penguins |> head()
```

More information about `penguins`, such as the units of each variable, can be found by typing `?datasets::penguins` into the console.
...
Using `tidyverse` functions on `penguins_2007`, create a new variable that contains the ratio of `bill_len` to `bill_dep` (call it `bill_ratio`). *Once you checked that your variable is created correctly*, overwrite `penguins_2007` so it contains this new variable.
...
Using `tidyverse` functions on `penguins_2007`, remove missing values for `sex`. Pipe a `ggplot` to create a single plot showing the distribution of `body_mass` colored by male and female penguins, faceted by species (use the function `facet_wrap()` with the option `nrow =` to give each species its own row). Which species shows the least sexual dimorphism (i.e., the greatest overlap of male/female size distributions)?

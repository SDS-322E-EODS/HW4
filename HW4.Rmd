...
First load the `tidyverse` package so that we can have access to the `tidyverse` data transformation functions. We also want to look at the `penguins` dataset from the `palmerpenguins` package. **IMPORTANT**: Please do NOT use the base R dataset that we have been using in lecture. The below code block will make sure you are loading the correct `penguins` dataset and should not need to be edited.
...
# Load palmerpenguins
# You may need to uncomment the next line to install the palmerpenguins package
# install.packages('palmerpenguins')
library(palmerpenguins)

# Save the data as a dataframe
penguins <- tibble(palmerpenguins::penguins)

# Take a look at the top of the data frame 
penguins |> head()
```

More information about `penguins` can be found by typing `?palmerpenguins::penguins` into the console.
...
Pipe `penguins_2007` to `ggplot()` to create a scatterplot of `body_mass_g_g` (y-axis) against `bill_ratio` (x-axis). Does it look like there is a relationship between the bill ratio and the body mass? *Note: you might see a Warning message.* What does this message refer to?
